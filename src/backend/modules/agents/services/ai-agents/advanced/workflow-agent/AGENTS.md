...
- Path: `packages/apps/ai-agents/src/services/ai-agents/advanced/workflow-agent`
...
- Functions (all):
  - `executeWorkflowAgent` (L5-L5, arrow)
- Exports:
  - `executeWorkflowAgent` (L5)
  - `* (re-export)` (L14)
  - `* (re-export)` (L15)
  - `* (re-export)` (L16)
  - `* (re-export)` (L17)
- Key snippets and use-case mapping:
  - `L5-L5`: Implements `executeWorkflowAgent` for this module use case.
