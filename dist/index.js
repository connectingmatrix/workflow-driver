...
const workflowQueueStatuses = new Map();
const workflowExecutionToRecord = new Map();
const workflowRunToProcess = new Map();
let workflowExecutorPubSub;
let workflowExecutorConfig;
let workflowQueueProducer;
let workflowQueueConsumer;
function workflowQueueProcessId(runId) { return `workflow:run:${runId}`; }
function workflowQueueUser(context) { return context.userId ?? context.organizationId ?? 'anonymous'; }
function workflowQueueScope(context) { return context.organizationId ? 'organization' : context.root ? 'global' : 'user'; }
function workflowQueueRequest(wf, executionId, runId, input, context) {
    const userId = workflowQueueUser(context);
    const workflowDefinition = { metadata: { ...wf.definition.metadata, id: wf.id, name: wf.name }, nodes: nodeArray(wf.definition), connections: edgeArray(wf.definition), edges: edgeArray(wf.definition), input };
...
function workflowQueueApply(event, context = {}) {
    const runId = String(event.runId ?? event.processId ?? makeId('run'));
    const executionId = String(event.executionId ?? runId);
    const workflowId = String(event.workflowId ?? (event.workflowReference && typeof event.workflowReference === 'object' ? event.workflowReference.workflowId : '') ?? 'unknown');
    ...
    const status = type === 'completed' || type === 'success' ? 'completed' : type === 'failed' || type === 'error' ? 'failed' : type === 'aborted' || type === 'cancelled' ? 'aborted' : type === 'queued' ? 'queued' : 'running';
    const log = event.log && typeof event.log === 'object' ? event.log : undefined;
    ...
    const next = { processId, executionId, runId, workflowId, status, progress: status === 'queued' ? 0 : status === 'running' ? 50 : 100, source: 'executor-kafka', updatedAt: nowIso(), logs: [...(previous?.logs ?? []), message].slice(-500), metadata: { event } };
    ...
    if (recordId) {
        ...
        try {
            execs.update(recordId, { status: mapped, logs: next.logs, output: status === 'completed' ? (event.result ?? event.payload) : undefined }, { root: true });
        }
        catch { }
    }
    ...
    if (status === 'completed')
        processMonitoring?.complete?.(processId, { workflowId, executionId, runId });
    if (status === 'failed')
        processMonitoring?.fail?.(processId, event.errorMessage ?? message, { workflowId, executionId, runId });
    if (status === 'aborted')
        processMonitoring?.abort?.(processId, message);
    void bus.emit('workflow:queue-status', next);
    return next;
}
async function ensureWorkflowQueueStarted() {
    if (!workflowExecutorPubSub)
        return;
    ...
    if (!workflowQueueConsumer && workflowExecutorPubSub.consumeWorkflowExecutionEvents) {
        workflowQueueConsumer = workflowExecutorPubSub.consumeWorkflowExecutionEvents({ applyExecutionEvent: (event) => { workflowQueueApply(event); }, config: workflowExecutorConfig });
...
async function executeWorkflowThroughQueue(id, input, context = {}) {
    ...
    if (!workflowQueueProducer)
        return originalWorkflowExecute(id, input, context);
...
Workflows.execute = executeWorkflowThroughQueue;
Object.assign(Workflows, {
    bindExecutorPubSub(executor, config) { workflowExecutorPubSub = executor; workflowExecutorConfig = config; return Workflows; },
    bindWorkflowExecutor(executor, config) { return Workflows.bindExecutorPubSub(executor, config); },
    ...
    applyExecutorQueueEvent(event, context = {}) { return workflowQueueApply(event, context); },
    executionQueueStatus(workflowId) { const rows = [...workflowQueueStatuses.values(), ...(workflowExecutorPubSub?.getRunning?.(workflowId ?? null) ?? []).map((row) => workflowQueueApply({ ...row, type: 'running' }))]; return workflowId ? rows.filter((row) => row.workflowId === workflowId) : rows; },
    queueStatus(workflowId) { return Workflows.executionQueueStatus(workflowId); },
    onWorkflowQueueStatus(handler) { return bus.on('workflow:queue-status', handler); },
    onWorkflowExecutionEvent(handler) { return bus.on('workflow:queue-status', handler); },
    async abortExecution(runIdOrProcessId, reason = 'aborted by user') { const status = workflowQueueStatuses.get(runIdOrProcessId) ?? [...workflowQueueStatuses.values()].find((row) => row.runId === runIdOrProcessId || row.executionId === runIdOrProcessId); const runId = status?.runId ?? runIdOrProcessId; const count = await workflowExecutorPubSub?.cancelRun?.(runId); if (status)
        workflowQueueApply({ ...status, type: 'aborted', message: reason });
    else
        processMonitoring?.abort?.(runIdOrProcessId, reason); return { runId, cancelled: count ?? 0, reason }; },
});
...
export const workflowSlashCommands = [{ command: '/workflow', owner: '@connectingmatrix/workflows', description: 'List/search/validate/execute/debug workflows', handler: (args, context) => Workflows.slash(args, context) }];
